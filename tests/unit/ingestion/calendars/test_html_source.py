...
def _make_source(cache, session=None, now=FIXED_NOW, stream=None, **overrides):
    settings = {
...
def test_maps_the_listing_fields(cache):
...
def test_start_time_is_localised_not_naive(cache):
...
def test_local_wall_clock_time_is_preserved(cache):
...
def test_event_link_becomes_the_candidate_url(cache):

    linked = _make_source(cache).fetch()[1]

    assert linked.title == "Alex Anthony"
    assert linked.url == "https://venue.test/e/1"


def test_unlinked_event_has_no_url(cache):

    assert _make_source(cache).fetch()[0].url is None


def test_a_useful_category_is_carried_as_data_not_as_a_description(cache):
...
def test_a_category_naming_two_activities_is_dropped(cache):
    ...
    candidate = _make_source(cache, session=_session(_response(page))).fetch()[0]

    assert candidate.description is None
    assert "listing_category" not in candidate.metadata


def test_the_other_category_is_dropped_as_noise(cache):
    page = (
        "<body><div>"
        '<p class="sqsrte-large"><strong>Wednesday, August 5</strong></p>'
...
def test_sports_is_carried_like_music(cache):
    page = (
        "<body><div>"
        '<p class="sqsrte-large"><strong>Wednesday, August 5</strong></p>'
...
def test_no_category_leaves_the_description_empty(cache):
    page = (
        "<body><div>"
        '<p class="sqsrte-large"><strong>Wednesday, August 5</strong></p>'
...
def test_ids_are_stable_across_fetches(cache):
    """The listing has no UID, so identity must derive from stable content."""

    first = _make_source(cache).fetch()
    later = _make_source(cache, now=FIXED_NOW + timedelta(days=1)).fetch()

    assert [c.id for c in first] == [c.id for c in later]


def test_ids_are_unique_within_a_page(cache):

    candidates = _make_source(cache).fetch()

    assert len({c.id for c in candidates}) == len(candidates)


def test_a_repeated_listing_line_yields_one_candidate(cache):
    ...
    candidates = _make_source(cache, session=_session(_response(repeated))).fetch()

    assert len(candidates) == 1


def test_id_changes_when_the_event_does(cache):
    """The fetch interval is disabled here so the cache cannot mask the change."""
    page = _PAGE.replace("Jazz Night", "Blues Night")

    original = _make_source(cache, min_fetch_interval_hours=0.0).fetch()[0]
    changed = _make_source(
        cache, session=_session(_response(page)), min_fetch_interval_hours=0.0
    ).fetch()[0]
...
def test_raw_published_at_is_never_set(cache):
    ...
    assert all(c.raw_published_at is None for c in _make_source(cache).fetch())


def test_end_time_is_absent_because_the_listing_has_none(cache):

    assert all(c.end_time is None for c in _make_source(cache).fetch())


def test_discovered_at_uses_the_injected_clock(cache):

    assert _make_source(cache).fetch()[0].discovered_at == FIXED_NOW


def test_the_listing_date_comes_from_local_time_not_utc(cache):
    ...
    candidates = _make_source(
        cache, session=_session(_response(page)), now=just_after_utc_midnight
    ).fetch()
...
class TestAuthoredSummary:
    ...
    def test_the_summary_is_composed_from_the_parsed_fields(self, cache):
        first = _make_source(cache).fetch()[0]

        assert first.summary == "Jazz Night at Joy Nest in Newburyport"
        assert first.metadata["authored_summary"] is True

    def test_a_missing_city_drops_the_in_clause(self, cache):
        page = (
            "<body><div>"
            '<p class="sqsrte-large"><strong>Wednesday, August 5</strong></p>'
            "<p><strong>Music</strong></p>"
...
class TestAuthoredKaraokeAndTrivia:
    ...
    def _fetch(self, cache, title):
        ...
        return _make_source(cache, session=_session(_response(page))).fetch()[0]

    def test_a_trivia_night_gets_authored_tags_and_no_karaoke(self, cache):
        candidate = self._fetch(cache, "Trivia")
    ...
    def test_a_karaoke_night_gets_authored_tags_and_no_trivia(self, cache):
        candidate = self._fetch(cache, "Karaoke")

        tags = {t.text for t in candidate.tags}
        assert "karaoke" in tags
        assert not any("trivia" in t for t in tags)

    def test_a_titled_variant_is_still_recognised(self, cache):
        candidate = self._fetch(cache, "Trivia w/ Lee Wolf")

        assert "trivia" in {t.text for t in candidate.tags}

    def test_an_unrecognised_line_under_the_heading_is_left_to_the_model(self, cache):
        """`DJ Bingo Night` appears under the same heading and is neither."""
        candidate = self._fetch(cache, "DJ Bingo Night")

        assert candidate.tags == []
        assert "authored_tags" not in candidate.metadata

    def test_a_music_event_is_never_given_authored_tags(self, cache):
        first = _make_source(cache).fetch()[0]
