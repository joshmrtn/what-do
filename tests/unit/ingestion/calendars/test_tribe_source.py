...
def _make_source(db, windows, max_requests=8, horizon_days=45, export_cap=2):
...
class TestWalking:
    ...
    def test_an_event_seen_twice_is_emitted_once(self, db):
        ...
        source, _ = _make_source(db, windows)

        assert len({c.id for c in source.fetch()}) == 2

    def test_the_walk_stops_when_nothing_new_arrives(self, db):
        repeated = _calendar(_vevent("a", "20260810"), _vevent("b", "20260812"))
        source, http = _make_source(db, {None: repeated, "2026-08-12": repeated})

        source.fetch()

        assert http.requested == [None, "2026-08-12"]

    def test_the_walk_stops_at_an_empty_window(self, db):
    ...
    def test_the_walk_stops_past_the_horizon(self, db):
        ...
        source, http = _make_source(db, windows, horizon_days=45)

        source.fetch()

        assert http.requested == [None]

    def test_the_walk_never_exceeds_its_request_cap(self, db):
        ...
        source, http = _make_source(db, windows, max_requests=3)
...
class TestMapping:
    def test_candidates_carry_the_feeds_identity(self, db):
        source, _ = _make_source(db, {None: _calendar(_vevent("a", "20260810"))})
