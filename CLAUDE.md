...
| Fetching from a provider for a date beyond its own horizon | The reply is empty, and **an empty reply is not cached** — so the request repeats on every run, for ever. Measured: 98 dates asked of Open-Meteo, 24 answered, **74 re-asked every batch and every rescore**; air quality had no bound and no cache at all, so all 98 hit a second endpoint each time. Bound every fetch by what the service can actually answer, and give each service its own bound — they do not share a range |
